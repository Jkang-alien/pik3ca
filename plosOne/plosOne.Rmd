...
Machine learning approaches can be applied to detect genomic alterations . Machine learning algorithms can build prediction models from a large number of predictors, such as radiomic  features @dercleIdentificationNonSmall2020, pathology image [@coudrayClassificationMutationPrediction2018] or gene expression data [@wayMachineLearningDetects2018 @loi*PIK3CA*MutationsAssociated2010]. Because most direct genomic tests are more specific and sensitive than predictive models, machine learning approaches may have limited roles in clinical practice, however, machine learning approaches are ideal when direct tests are unavailable or fail.  
...
The top 30 important predictors are shown (Figure 3). The coefficient is the parameter of the predictor which represents the effect of the predictor on prediction. *insulin like growth factor 1 Receptor* (*IGF1R*)  mRNA expression was the strongest negative predictor, and *PTEN* was the strongest positive predictor. Both *IGF1R*  and *PTEN* are key players in the tyrosine kinase pathway . The cancer types were important predictors. Some cancer types including uterine carcinosarcoma (UCS), bladder urothelial carcinoma (BLCA), pancreatic adenocarcinoma (PAAD), lymphoid neoplasm diffuse large B-cell lymphoma (DLBC) were the strongest predictors. 
...
Several studies have attempted to predict genomic alterations from gene expression data [@cizkovaGeneExpressionProfiling2010; @loi*PIK3CA*MutationsAssociated2010]. A study investigated *PIK3CA* mutation predictions using gene-expression signatures which is a sum of the average of the logarithmic gene expression . The model showed good performance AUROC 0.71 in an independent test set  [@cizkovaGeneExpressionProfiling2010; @loi*PIK3CA*MutationsAssociated2010].Another study predicted copy number alterations with gene expression, using a multinomial logistic regression model with least absolute shrinkage and selection operator (LASSO) parameters [@muCNAPEMachineLearning2019]. The prediction of the 1p/19q codel  was very good, with an AUROC of 0.997, and gene-level predictions were good, with an AUROC of 0.75 [@muCNAPEMachineLearning2019]. A Hidden Markov Model Approach for Prediction of Genomic Alterations from Gene Expression Profiling [@gengHiddenMarkovModel2008]. A logistic regression model was used for MYCN  gene amplification in neuroblastoma [@heGeneSignaturesAssociated2020].
